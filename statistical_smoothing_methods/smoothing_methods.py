...
y = y.fillna(y.bfill()) #eksik değerin bir sonraki değerini eksik değer için kullan
...
y_pred = ses_model.forecast(48) #test setinin boyutu kadarlık forecast yapıyor olacağız

mean_absolute_error(test, y_pred) #tahmin değerleri ile gerçek değerleri kıyaslamak için

mean_squared_error(test, y_pred, squared=False) #RMSE
...
des_model.summary()
des_model.resid #artıklar

# Lijung-Box testini uygulayın
import numpy as np
import statsmodels.api as sm
from statsmodels.stats.diagnostic import acorr_ljungbox
acorr_ljungbox(des_model.resid,lags=48) #pvaluelarıb hepsi H0 red artıkların otokorelasyon problemi var
...
#aylık veri old için seas period 12
...
def tes_optimizer(train, abg, step=48):
    best_alpha, best_beta, best_gamma, best_mae = None, None, None, float("inf") #hata değerine sonsuz değeri atandı
    for comb in abg:
...
final_tes_model = ExponentialSmoothing(train, trend="add", seasonal="add", seasonal_periods=12).\
            fit(smoothing_level=best_alpha, smoothing_trend=best_beta, smoothing_seasonal=best_gamma) #ya da smoothing_slope =best_beta şeklinde de yazabilirsin
