...
arima_model.summary() #Model: ARIMA(1, 1, 1) model derecelerinin ayarlandığı kısım

y_pred = arima_model.forecast(48)
y_pred = pd.Series(y_pred, index=test.index) #görselleştirme fonks series şeklinde girdi istediği için formatını series'e çevirelim
...
feature_predict #elimizde olmayan veriyi predict ettik.

#Biz train test ayrımını model validasyonu için yapmıştık. Doğru yolda olup olmadığımızı görmek için, hiperparametre optimizasyonu için yaptık. Dolayısıyla işlemlerimiz tamamlandığından bütün veriye tekrardan fit edebiliriz. Bütün veriye tekrar fit etmeliyiz.Daha zengin bir veriyle daha kuvvetli bir model olacağını öngörerek. Ee zaten bu verinin başka parçlarında belirli bir örüntüyü barındırdığını varsaydığımız bir parçada parametreleri bulduk. Dolayısıyla bütün veriyle final modelimi oluşturuyorum ve gelecekle ilgili tahminde bulunuyorum.
